...
message Stats{
     map<string,string> connectedPeer=1;
}
`)
class Chat
{
   ...
   constructor(libp2p,topic,messageHandler)
   {
      ...
      this.conn=
      {
       ...
       this.libp2p.connectionManager.on('peer:connect', async (connection) => {
        try{ 
          await this.sendStats(this.conn)
        }catch(err)
        {
          console.error(err);
        }
          ...
          switch (request.type) {
            case Request.Type.SEND_MESSAGE:
              ...
              case Request.Type.STATS:
                if(this.name.has(request.stat.connectedPeer.name)) return;
                   this.name.set(request.stat.connectedPeer.name,request.stat.connectedPeer.cid);
